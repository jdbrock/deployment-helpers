...
<#
    ...
    .PARAMETER overwrite
        overwrite an existing app of the same version, otherwise uploads a new one.
#>
function Push-AppleToHockeyApp {
    [CmdletBinding()]
    param (
		#[parameter(Mandatory=$true, position = 0)][string] $filePattern,
        [parameter(Mandatory=$true, position = 0)][string] $apiKey,
        [parameter(Mandatory=$true, position = 1)][string] $appId,
        [parameter(Mandatory=$false)][int[]] $teams,
        [parameter(Mandatory=$false)][int[]] $users,
	...
	Get-ChildItem -Include "*.ipa" -Recurse | 
		Foreach-Object {
			Push-ToHockeyApp $_.FullName @psBoundParameters
		}
}
...
function Push-AppleToHockeyAppAuto {
	...
	Get-ChildItem -Include "*.ipa" -Recurse | 
		Foreach-Object {
			Push-ToHockeyApp $_.FullName @params
		}
}
...
export-modulemember -function Push-AppleToHockeyApp
export-modulemember -function Push-AppleToHockeyAppAuto